...
export async function GET(request: NextRequest) {
  try {
    ...
    // Solo ADMIN y CLINIC_ADMIN pueden acceder
    if (
    ...
    const doctorId = searchParams.get("doctorId"); // null = todos
    const appointmentTypeId = searchParams.get("appointmentTypeId"); // null = todos
    const clinicIdParam = searchParams.get("clinicId"); // null = todas (solo para ADMIN)
    ...
    // Filtro por clínica
    if (user.role === "ADMIN") {
      // Si es ADMIN y seleccionó una clínica específica, filtrar por esa
      if (clinicIdParam && clinicIdParam !== "all") {
        where.clinicId = clinicIdParam;
      }
      // Si no seleccionó clínica, mostrar todas (no agregar filtro)
    } else {
      // Si no es ADMIN, filtrar por su clínica
      if (user.clinicId) {
